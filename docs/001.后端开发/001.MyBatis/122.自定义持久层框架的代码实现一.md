...
meta:
  - name: keywords
    content: 配置文件 项目 结构 具体 代码 mybatis-3 mybatis
  - name: description
    content: 本文介绍了一个基于Java和Kotlin的MyBatis框架示例项目的结构和关键代码部分，包括配置文件、Mapper文件、资源处理类以及SqlSessionFactory的构建对象。示例项目用于展示如何配置MyBatis，读取资源，构建SqlSessionFactory，以便进行数据库操作。
tags:
  - 配置文件
  - 项目
  - 结构
...
categories:
  - 后端开发
  - MyBatis
permalink: /post/122-customized-code-of-long-lasting-layer-framework-4j49n.html
author:
  name: terwer
