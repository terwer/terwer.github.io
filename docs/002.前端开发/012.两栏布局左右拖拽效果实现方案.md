...
date: 2022-07-19 06:21:46
permalink: /post/two-column-layout-left-and-right-dragging-effect-implementation-plan.html
meta:
  - name: keywords
    content: section layout drag
  - name: description
    content: 两栏布局左右拖拽效果实现方案css方式实现第一种利用的css的方式实现，利用浏览器非overflowauto元素设置resize可以拉伸的特性实现无javascript的分栏宽度控制。webkit浏览器下滚动条可以自定义，其中resize区域大小就是scrollbar的大小，于是，我们可以将整个拉伸区域变成和容器一样高红色的就是扩大后的区域不推荐使用css的方式实现，毕竟兼容性并不好。在火狐上表现不佳。&lt!doctypehtml&gt&lthtmllangen&gt&lthead&gt&ltmetac
categories:
  - 前端开发
tags:
  - section
